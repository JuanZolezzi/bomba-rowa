...
const slides = document.querySelectorAll('.slider-img1');
const totalSlides = slides.length;
const slidesToShow = 5; // Número de imágenes visibles al mismo tiempo
const slideWidth = slides[0].clientWidth + parseInt(window.getComputedStyle(slides[0]).marginRight, 10);

// Clonar las primeras y últimas imágenes para crear el efecto de loop infinito
for (let i = 0; i < slidesToShow; i++) {
    const cloneFirst = slides[i].cloneNode(true);
    const cloneLast = slides[totalSlides - 1 - i].cloneNode(true);
    sliderWrapper.appendChild(cloneFirst); // Añade al final
    sliderWrapper.insertBefore(cloneLast, sliderWrapper.firstChild); // Añade al inicio
}
...
function moveSlide(step) {
    ...
    // Verificación para loop infinito
    if (slideIndex >= totalSlides) {
        setTimeout(() => {
            sliderWrapper.style.transition = 'none';
            slideIndex = 0; // Reinicia el índice
            updateSlidePosition();
        }, 500); // El tiempo coincide con la duración de la transición
    } else if (slideIndex < 0) {
        setTimeout(() => {
            sliderWrapper.style.transition = 'none';
            slideIndex = totalSlides - 1; // Va al final
            updateSlidePosition();
        }, 500);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    slideIndex = slidesToShow; // Inicia con el primer set de imágenes visibles
    sliderWrapper.style.transform = `translateX(${-slideIndex * slideWidth}px)`;
});
